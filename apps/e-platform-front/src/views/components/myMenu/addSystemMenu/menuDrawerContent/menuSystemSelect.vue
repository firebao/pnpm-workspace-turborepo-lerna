...
<!--          >-->
<!--          </treeselect>-->
          <el-select v-model="formInline.majorId"  style="height: 30px; width: 160px" clearable @change="handleMajorSelect" size="small"
                     placeholder="请选择专业">
            <el-option v-for="item in majorOptions" :label="item.majorName"
...
export default {
  ...
  methods: {
      ...
      await searchMajorList(params).then((res) => {
        if (res.status === 200) {
          ...
          if(this.majorOptions.length > 0) {
            ...
            this.getSearchSystemList(this.formInline.majorId)
          } else {
      ...
      if (val) {
        await searchSystemList(params).then((res) => {
          if (res.status === 200) {
            ...
            // this.systemLists.forEach(item =>{
            //   item.systemName = '数据服务'
            // })
             const  inf = this.systemLists.findIndex(item => item.systemName== '数据服务')
            if(inf>-1){
              this.systemLists.splice(inf,1)
            }
          }
        })
      } else {
    ...
    handleReset(){
      this.getSystemDescription()
      this.formInline = {
        majorId: this.majorOptions[0].majorId, // 专业id
        majorName: this.majorOptions[0].majorName, // 专业id
        layerId: null, // 层级id
        menuSystemId: this.systemLists[0].systemId, // 系统id
        systemName: this.systemLists[0].systemName,
        menuName: '',
        systemDescription: '',
